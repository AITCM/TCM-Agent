...
3.  **核心页面**: 大气的首页，专业的Agent对话页（支持Markdown渲染），响应式导航栏和页脚。
4.  **数据可视化**: 
    *   **Agent页面**: 聊天界面右侧面板可以**同时展示富集分析结果图片和中药靶点知识图谱**。当两种可视化都触发时，它们会**垂直排列**显示，互不干扰。
    *   **可拖拽分隔线**: 右侧面板中的图片和图谱之间设有可拖拽分隔线，用户可以通过拖拽来调整两个组件的高度比例，轻松放大或缩小富集分析图或知识图谱，实现最佳查看体验。
...
3.  **访问**: 浏览器打开 `http://localhost:3001` (或其他 npm start 输出的地址)

## 仓库状态说明（闭源）

本项目当前已设置为闭源：代码仓库的源代码已从公开分支中移除，仅保留本说明文档。

如果有技术合作或合作开发意向，请联系：北京中医药大学 DeepTCM 中医智慧深度探索实验室，邮箱 wangxt@bucm.edu.cn。
## 前端功能亮点
